...
for i,start in enumerate(range(0,144,12)):
 for j,note in enumerate(chords[i%4]):tone(start,16,note,.020,(-.6+j*.4))
for start in np.arange(40,60,.75):tone(float(start),1.5,[72,79,74,67][int((start-40)/.75)%4],.014,pan=.25,soft=False)
for start,note in [(7,79),(20,74),(29,72),(60,79),(64,84),(71,79),(79,76),(98,74),(105,67),(115,79),(122,84),(130,76),(143,72)]:tone(start,3.5,note,.022,pan=-.2,soft=False)
...
out=Path(__file__).resolve().parents[1]/'build/film-score.wav';out.parent.mkdir(exist_ok=True)
