...
export function lightProduct(scene,renderer){
 const before=new Set(scene.children);scene.add(new T.HemisphereLight('#eaf1fb','#22252b',.68));
 ...
 const lights=scene.children.filter(o=>!before.has(o));return {dispose:(contextLost=false)=>{if(!contextLost)target.dispose();for(const l of lights)scene.remove(l);}};
}
