...
import {makeObstacle} from './interaction.js';
...
export function makeScan(anchor,head=null){
 ...
 let surface;
 if(head){surface=head;target.rotation.y=0;head.traverse(n=>{if(n.isMesh)n.material=material;});target.add(head);material.color.set('#aab8bf');material.metalness=.06;material.roughness=.58;}
 else {
...
export function makeEcho(anchor,sculpted=false){
 ...
 const normal=hit.face.normal.clone().transformDirection(wall.matrixWorld);
 const localNormal=normal.clone().transformDirection(anchor.matrixWorld.clone().invert());
 const impactRotation=new T.Quaternion().setFromUnitVectors(new T.Vector3(0,0,1),localNormal);
 const glow=new T.Mesh(new T.PlaneGeometry(.041,.041),new T.ShaderMaterial({transparent:true,depthWrite:false,side:T.DoubleSide,uniforms:{strength:{value:0}},vertexShader:'varying vec2 q;void main(){q=uv-.5;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}',fragmentShader:'varying vec2 q;uniform float strength;void main(){float r=length(q);float a=exp(-r*r*24.)*.32+exp(-pow((r-.27)*35.,2.))*.5;gl_FragColor=vec4(.65,.87,1.,a*strength);}'}));glow.position.copy(point);glow.position.add(localNormal.clone().multiplyScalar(.0006));glow.quaternion.copy(impactRotation);root.add(glow);
 const planePoint=anchor.localToWorld(point.clone()),planeNormal=normal.clone();
