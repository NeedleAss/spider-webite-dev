...
export function faceFollow(t){
 ...
 return {active:t>=28&&t<38,targetZ:path(t),robotZ:path(t-.65),label:t<28?'人脸已确认':t<30.65?'重新确认 · 向左跟随':t<34.15?'持续确认 · 向右跟随':t<37.65?'保持目标 · 跟随回位':'位置稳定 · 停稳'};
}
...
export function evaluate(time,{aspect=16/9,reduced=false}={}){
 ...
 if(follow.active){pose.position[2]=follow.robotZ;pose.wheels=wheelAngles(0,-follow.robotZ,0);pose.label=follow.label;}
 ...
 return {t,contentTime,follow,index,chapter:s.id,progress:clamp((t-s.start)/(s.end-s.start)),...camera,open,pose,care,gesture,oled,copy:s,reduced,console:s.id==='console',focus:s.id==='vision'?'camera':s.id==='range'&&contentTime<35.3?'ultrasonic':null};
}
