...
void followTests() {
  ...
  VisionPacket no;no.seq=5;f.update(no);assert(f.lost());assert(f.output().vx==0);
...
void imuTests() {
  ...
  f.update(.6f,0,.8f,0,0,0,5110);assert(f.state().tiltFault);
  for(int t=5120;t<7100;t+=10)f.update(0,0,1,0,0,0,t);
