...
int main() {
  ...
  }
  {
    ...
    s.tick(10+SafetyController::CommandExpiryMs); assert(s.snapshot(10+SafetyController::CommandExpiryMs).mode == Mode::Manual); zero(s,10+SafetyController::CommandExpiryMs);
  }
  {
  ...
  }
  {
    ...
    s.network(false,1804); zero(s,1804); assert(s.snapshot(1804).mode == Mode::Idle); s.network(true,1805);
  ...
  for (int i=0;i<20;++i) {
    ...
    s.tick(1800); zero(s,1800);
    assert(s.snapshot(1800).mode == ((i%4==0||i%4==2)?Mode::Idle:Mode::Manual));
  }
  std::cout << "Safety controller: expired input zeros; owner/network loss cancels; arbitration preserved\n";
}
