async page=>{
 ...
 for(const t of [22,43,85,91]){await api('seek',t);const a=await canvas.screenshot();await api('seek',109);await api('seek',t);ok('Reverse seek reproduces rendered sensor/hand frame '+t,a.equals(await canvas.screenshot()));}
 ...
 await api('seek',69);ok('Sculpted gesture replaces flat symbol',await page.locator('#gestureCard').isHidden()&&(await snap()).interaction.visible&&(await snap()).interaction.gesture==='TWO');
 await api('setMode','deck');await api('seek',22);await page.locator('#next').click();await page.waitForFunction(()=>__careRover.snapshot().time>38);ok('Deck next chapter plays correct segment',(await snap()).time<48);
 ...
 await page.keyboard.press('ArrowDown');ok('Keyboard down advances from a focused transport button',(await snap()).time>=48);await page.locator('#pause').click();
