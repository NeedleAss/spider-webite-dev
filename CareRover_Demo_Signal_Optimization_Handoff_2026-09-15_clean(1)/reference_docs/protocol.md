...
- 手势：`NONE`, `PALM`, `FIST`, `THUMB_UP`, `VICTORY`, `POINT_LEFT`, `POINT_RIGHT`, `ONE`, `TWO`, `THREE`, `FOUR`, `FIVE`, `OK`, `CALL`, `LIKE`, `DISLIKE`, `UNKNOWN`。显示置信度和更新时间；stable 且 confidence ≥ 0.45 显示“已稳定”。运动动作由主控固件消费稳定手势边沿，网页只显示结果与机器人模式，不直接把手势转换为速度包。
...
跟随目标为人脸框，进入后用三个匹配新框的面积中位数记录近似距离；前后速度维持框面积，旋转速度维持框中心，两者同时闭环。不提供身份识别、米制距离、实测轮速或自动搜索。目标阈值为 0.45，来源超过 490 ms 无合格更新仍停止。全部运动经主控安全仲裁；网页不是物理 watchdog。

稳定手势动作：显示仍采用 0.45 的低阈值，但运动动作要求至少 `80×120` 的近景手框；`LIKE/TWO/OK` 连续 4 个动作确认帧才触发，`DISLIKE` 停止为 2 帧，同一手势离场 3 帧后才可重触发。`LIKE` 请求 `PERSON_FOLLOW`（若手势与人物帧交错，可等待下一张合格人物帧最多 2 s），`DISLIKE` 停止并回 `IDLE`，`TWO` 顺时针旋转约 360°，`OK` 逆时针旋转约 360°。旋转角由 MPU6050 相对 yaw 累计，跨 ±180°连续处理；完成或 12 s 超时即停止。网页仍可随时 ESTOP。
