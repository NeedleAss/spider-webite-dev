...
| 舵机与 NVS（新增/合并参考） | `continuous_servo_drive.*`、`omni_kinematics.h`、`motion_layout.h`、`motion_calibration.h`（主控目录） | 实际四路 LEDC、正交全向轮分配、每轮 neutral/invert/span、布局版本/verified 门禁、正常缓动/安全中值；无编码器闭环 |
...
- 现场最终轮位：以车头为上，FL/FR/RL/RR 为 GPIO10/12/13/11（俯视顺时针 GPIO10→12→11→13）。这不是 X-drive：GPIO10/11 为前后主动轴，GPIO12/13 为左右主动轴，小滚轮提供垂直方向被动随动；vx 前进、vy 右移、wz 顺时针。经实车顺逆旋转修正后的权威分配为 `[vx+wz, vy+wz, vy-wz, vx-wz]`，具体电气轮向仍必须实测校准。
