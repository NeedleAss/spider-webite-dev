...
这是从 Mac 导出的集成工程，不是空白项目。开始工作先读 `WINDOWS_START_HERE.md`，再读 `docs/WINDOWS_AI_HANDOFF.md`、`docs/tracking-development.md` 和 `docs/tracking-acceptance.md`。本文件适用于此工程；原始参考目录保持不变。

- 当前根目录 `firmware/`、`js/`、`tools/` 是开发代码；`CareRover_Tracking_Motion_Handoff_2026-09-11/` 是原始参考，不能整目录覆盖回来。
- 本机软件构建/主机测试通过，Windows 和实物证据仍未取得。不要把 Mac PASS、模拟遥测或控制输出当作实物验收。
- 先核验清单、记录 Windows 环境与设备、备份，再按 CAM → observe → 架空校准 → manual → follow → 无 USB 十分钟的顺序推进。详细阶段门槛见交接文档。
