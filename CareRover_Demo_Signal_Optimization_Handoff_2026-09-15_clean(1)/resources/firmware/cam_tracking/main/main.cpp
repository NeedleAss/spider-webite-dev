...
#endif
    false);
  faces.set_score_thr(0.42f,0);
#if !CONFIG_CAREROVER_PICO_FACE
  faces.set_score_thr(0.42f,1);
#endif
#endif
  ...
  for(;;) {
    ...
    if(faceTurn) {
      ...
      for(const auto& result:results) {
        auto candidate=box(result,'P');if(!candidate.found||candidate.score<450)continue;
