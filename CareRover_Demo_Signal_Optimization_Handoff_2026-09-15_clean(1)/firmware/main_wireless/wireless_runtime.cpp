...
GestureActionLatch gestureActions;
...
void imuTask(void*) {
  ...
  for(;;) {
    ...
    portENTER_CRITICAL(&safetyMux);
    safety.imu(sample.valid,sample.calibrated,sample.tiltFault,now);
    if(sample.valid&&sample.calibrated&&!sample.tiltFault) safety.updateGestureTurn(sample.yaw,now);
    portEXIT_CRITICAL(&safetyMux);
...
void wirelessGesture(const char* label,float score,bool accepted,bool actionEligible,uint32_t inferMs) {
  const auto now=wirelessNowMs();
  ...
  const auto action=gestureActions.update(actionEligible,upper);
...
void wirelessVision(const carerover::VisionPacket& p,bool resync) {
  ...
  if(p.kind=='P') {
    ...
    if(pendingGestureFollowUntil) {
      if(p.receivedMs>pendingGestureFollowUntil) pendingGestureFollowUntil=0;
